#ifndef _INCLUDED_HTTPD_H_
#define _INCLUDED_HTTPD_H_
#include <string>
#include <map>
#include <list>
#include <vector>
...
namespace httpd {
    ...
    typedef std::map<std::string, std::string, CaseInsensitiveLess> HttpHeaderMapType;
    ...
    public:
        ...
        virtual void includeHandler(const std::string &reference, HttpContext *context) = 0;
    };

    class HttpServerPageHandler {
    public:
        virtual ~HttpServerPageHandler() {}
        virtual void pageHandler(const std::string &path, HttpContext *context) = 0;
    };
    ...
    class HttpServerException : public virtual std::exception {
    public:
        /**
    ...
    public:
        ...
        HttpServer(uint16_t port, const std::string &rootDirectory, bool ipv6 = false);
        ...
        virtual void registerContentType(const std::string &extension, const std::string &type);

        virtual void registerVirtualDirectory(const std::string &path, const std::string &directory);

        virtual void registerIncludeHandler(const std::string &ref, HttpServerIncludeHandler *handler);

        virtual void registerPageHandler(const std::string &path, HttpServerPageHandler *handler);
