...
namespace httpd {
    ...
    const char *HttpServerException::what() const throw() {
        return message.c_str();
    }

    LoggerPtr HttpServer::logger(Logger::getLogger("httpd.Httpserver"));
    ...
    const string &HttpServer::getRootDirectory() const {
        return rootDirectory;
    }

    bool HttpServer::isIPv6() const {
        return ipv6;
    }

    void HttpServer::registerContentType(const string &extension, const string &type) {
        if (extension.size() > 0 && extension[0] == '.') {
            // The extension should have no dot
            throw HttpServerException("Extension should not have dot at the beginning.");
        }
        contentTypeMap[extension] = type;
    }

    void HttpServer::registerVirtualDirectory(const string &path, const string &directory) {
    ...
    void HttpServer::registerIncludeHandler(const string &ref, HttpServerIncludeHandler *handler) {
    ...
    void HttpServer::registerPageHandler(const string &path, HttpServerPageHandler *handler) {
        if (path.size() == 0 || path[0] != '/') {
            throw HttpServerException("Path must be an absolute one.");
        }
    ...
    void HttpServer::registerAccessHandler(HttpServerAccessHandler *handler) {
        accessHandlers.push_back(handler);
    }

    void HttpServer::setSessionLife(uint32_t sessionLife) {
        this->sessionLife = sessionLife;
    }

    void HttpServer::cleanExpiredSession() {
    ...
    void HttpServer::generateSession(const string &sessionId, HttpContext *context) {
        LOG4CXX_INFO(logger, "New session with SID=" << sessionId);
        context->response.headerMap["Set-Cookie"] = "SID=" + sessionId + "; path=/";
    ...
    void HttpServer::getSession(HttpContext *context) {
        // Clean the session first
        cleanExpiredSession();

        HttpCookieMapType::const_iterator iter = context->request.cookieMap.find("SID");
        if (iter == context->request.cookieMap.end() || iter->second.size() == 0) {
            ...
            string sessionId;
            for (int i = 0; i < 16; ++i) {
                sessionId += possible[rand() % (sizeof(possible) / sizeof(char) - 1)];
            }
