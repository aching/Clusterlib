...
void checkEqual(const JSONValue &v1, const JSONValue &v2) {
    ...
    if (v1.type() == typeid(JSONValue::JSONNull)) {
        BOOST_CHECK(v1.get<JSONValue::JSONNull>() == v2.get<JSONValue::JSONNull>());
    } else if (v1.type() == typeid(JSONValue::JSONInteger)) {
        BOOST_CHECK_EQUAL(v1.get<JSONValue::JSONInteger>(), v2.get<JSONValue::JSONInteger>());
    } else if (v1.type() == typeid(JSONValue::JSONFloat)) {
        BOOST_CHECK_CLOSE(v1.get<JSONValue::JSONFloat>(), v2.get<JSONValue::JSONFloat>(), 1e-9);
    } else if (v1.type() == typeid(JSONValue::JSONBoolean)) {
        BOOST_CHECK_EQUAL(v1.get<JSONValue::JSONBoolean>(), v2.get<JSONValue::JSONBoolean>());
    } else if (v1.type() == typeid(JSONValue::JSONString)) {
        BOOST_CHECK_EQUAL(v1.get<JSONValue::JSONString>(), v2.get<JSONValue::JSONString>());
        ...
        BOOST_CHECK_EQUAL(a1.size(), a2.size());
        for (JSONValue::JSONObject::const_iterator iter = a1.begin(); iter != a1.end(); ++iter) {
